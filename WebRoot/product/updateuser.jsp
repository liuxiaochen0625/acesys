<%@ page language="java" import="java.util.*,com.ascent.po.*" pageEncoding="GB2312" %>
...
<body>
<form name="" method="post" action="<%=request.getContextPath()%>/updateusrManagerAction.action">
<div class="head">
	<div id="login_1"></div>
	<div id="login_2"></div>
	<div id="login_3"></div>
...
<div class="head">
  <div class="left_products">
	...
	<% 
		...
		Usr usr = (Usr)session.getAttribute("UID_Productuser");
	%>
<div class="right_proaducts">我的位置&gt;&gt;电子商务管理&gt;&gt;用户修改 </div>
	...
	<table width="470" border="0" cellspacing="0" bordercolor="#9EA7AB" bgcolor="#DFEFFD">
      ...
      <td height="10" colspan="4"></td>
	 	...
	 	<div> <input type="hidden" name="uid" value="<%=usr.getId()%>"/> </div>
	  <tr>
	  ...
	  <%
	   ...
	   <%
	    ...
	    %>
	   <input name="city" type="text" value="<%=usr.getCity()%>" />
      <%
	   	}
	    %>
	  </td>
      ...
      <td width="155" class="table_c"><input name="username" type="text" value="<%=usr.getUsername()%>" /></td>
	  ...
	  <%
	   ...
	   <%
	    ...
	    %>
	    <input name="fullname" type="text" value="<%=usr.getFullname()%>" />	  </td>
      <%
	   	}
	    %>
      <td width="70" class="table_c"><div class="table_hui" id="result_box" dir="ltr">
      ...
      <%
	  	if(usr.getTitle()==null){
	   %>
	   <input name="title" type="text" value="" />
	   <%
	   	}else{
	    %>
	    <input name="title" type="text" value="<%=usr.getTitle()%>" />
      <%
	   	}
	    %>
      </td>
	  ...
	  <%
	   ...
	   <%
	    ...
	    %>
	   <input name="tel" type="text" value="<%=usr.getTel()%>" />	  
      <%
	   	}
	    %>
	    </td>
	  ...
	  <td height="30" class="table_c"><input name="password" type="password" value="<%=usr.getPassword()%>" /></td>
	  <tr>
	  ...
	  <td height="25" class="table_c">
	   ...
	   <%
	    ...
	    %>
	   <input name="zip" type="text" value="<%=usr.getZip()%>" />	  	  
      <%
	   	}
	    %>
	    </td>
	  ...
	  <td height="25" class="table_hui"><span class="table_c">
	   ...
	   <%
	    ...
	    %>
	   <input name="job" type="text" value="<%=usr.getJob()%>" />  	  
      <%
	   	}
	    %> 
	...
	<tr>
	  ...
	  <td height="30" class="table_c"><input name="email" type="text" value="<%=usr.getEmail()%>"  /></td>
	 ...
	 <%
	   ...
	   <%
	    ...
	    %>
	    <input name="country" type="text" value="<%=usr.getCountry()%>" />  	  
      <%
	   	}
	    %> 
	 ...
	 <tr>
	  ...
	  <%
	   ...
	   <%
	    ...
	    %>
	     <input name="companyname" type="text" value="<%=usr.getCompanyname()%>"/>	  
      <%
	   	}
	    %> 
	 </td>
	 ...
	 <tr>
      <td height="25" colspan="4" class="table_hui"><span class="table_hui">&nbsp;&nbsp;公司地址: </span>
        <%
	  	if(usr.getCompanyaddress()==null){
	   %>
	   <input name="companyaddress" type="text" value="" />
	   <%
	   	}else{
	    %>
	     <input name="companyaddress" type="text" value="<%=usr.getCompanyaddress()%>" />
      <%
	   	}
	    %>  
      </td>  
      ...
      <td height="25" colspan="4" class="table_hui"><span class="table_hui">备注:</span>
	   ...
	   <%
	    ...
	    %>
	    <textarea name="note" cols="70" rows="8"> <%=usr.getNote()%></textarea>
      <%
	   	}
	    %>
       </td>
